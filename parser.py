...
def parse_schedule_data(
    raw_data: dict, team_ids: list, parent_club_cache: dict
) -> list:
    ...
    if not dates:
        return [
            {"team": tid, "gameInfo": info} for tid, info in results.items()
        ]  # return empty object if no games
    ...
    for game in games:
        ...
        for tracked_id in [away_id, home_id]:
            ...
            if tracked_id in results:
                ...
                opponent_parent = parent_club_cache.get(
                    opponent_id, "Unknown MLB Affiliate"
                )  # get parent club from cache
                ...
                if game_state == "Not Started":
                    ...
                    if away_pitcher and home_pitcher:
                        game_info["probablePitchers"] = (
                            f"{away_pitcher} vs {home_pitcher}"
                        )
                    elif away_pitcher:
                        game_info["probablePitchers"] = (
                            f"{away_pitcher} (Away) - Home TBD"
                        )
                    elif home_pitcher:
                        game_info["probablePitchers"] = (
                            f"Away TBD - {home_pitcher} (Home)"
                        )
                    else:
                    ...
                    if tracked_id == away_id:
                        game_info["finalScore"] = (
                            f"{our_team_name} {away_score} - {opponent_name} {home_score}"
                        )
                    else:
                        game_info["finalScore"] = (
                            f"{our_team_name} {home_score} - {opponent_name} {away_score}"
                        )

                    decisions = game.get("decisions", {})

                    game_info["winningPitcher"] = decisions.get("winner", {}).get(
                        "fullName", "Unknown"
                    )
                    game_info["losingPitcher"] = decisions.get("loser", {}).get(
                        "fullName", "Unknown"
                    )

                    if "save" in decisions:
                        game_info["savePitcher"] = decisions.get("save", {}).get(
                            "fullName"
                        )
                    ...
                    game_info["inning"] = (
                        f"{linescore.get('inningHalf', '')} {linescore.get('currentInningOrdinal', '')}"
                    )
                    ...
                    game_info["batterUp"] = offense.get("batter", {}).get(
                        "fullName", "Unknown"
                    )
                    game_info["currentPitcher"] = defense.get("pitcher", {}).get(
                        "fullName", "Unknown"
                    )
                    ...
                    if "third" in offense:
                        runners.append("3B")
                    game_info["runnersOnBase"] = (
                        ", ".join(runners) if runners else "Bases Empty"
                    )

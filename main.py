...
async def fetch_affiliate_ids() -> list:
    ...
    params = {"teamIds": MARLINS_TEAM_ID, "year": date.today().year}
        ...
        try:
            ...
            for team in teams:
                ...
                if team_id and team_id != MARLINS_TEAM_ID:
                    affiliate_ids.append(
                        str(team_id)
                    )  # add affiliates to Marlins from cache

                if sport_id:
                    SPORT_IDS_CACHE.add(str(sport_id))

            logger.info(
                f"Successfully loaded {len(affiliate_ids)} affiliate IDs across {len(SPORT_IDS_CACHE)} sport levels"
            )
        ...
        try:
            ...
            for team in teams:
                ...
                if parent_org_name:
                    ...
                    TEAM_TO_PARENT_CLUB_CACHE[team_id] = team.get(
                        "name"
                    )  # no parent means that they are an mlb team?

            logger.info(
                f"Successfully cached {len(TEAM_TO_PARENT_CLUB_CACHE)} team parent-club mappings"
            )
...
async def get_schedule(target_date: date = None):
    ...
    parsed_data = parse_schedule_data(
        raw_schedule_data, TEAM_IDS_CACHE, TEAM_TO_PARENT_CLUB_CACHE
    )
